...
" ----------------------------------------------------------------------------
" ----------------------------------------------------------------------------
"  Personal perl specific settings
" ----------------------------------------------------------------------------
" ----------------------------------------------------------------------------
" ----------------------------------------------------------------------------
"
...
let g:Perl_AuthorName      = 'Vikas Dhiman'     
let g:Perl_AuthorRef       = 'dhimanv'                         
let g:Perl_Email           = 'dhimanv@deshaw.com'            
let g:Perl_Company         = 'Deshaw & Co'

" Load or unload perl menus
let g:Perl_LoadMenus       = 'yes'

"""""""""""""""""""""Perl Explorer"""""""""""""""""""""

noremap <Leader>jm :w<CR>:e `/prod/tools/base/bin/share/desperldoc -l <cword>`<CR>

"""""""""""""""""""""""""""""""""""""""""""""""""""""""

set textwidth=78
map <F3> :call Perl_SyntaxCheck()<CR>:redraw!<CR>:call Perl_SyntaxCheckMsg()<CR>
...
"Do not include 'included' files in ins-complete. (it makes it slow)
"Do not include tags. Tag files have grown big in size
set complete-=i
set complete-=t

" include dictionary for ins-complete
set complete+=k

" Treat Deshaw::Energy as two separate words.
" set iskeyword-=:

" Show module in preview window
noremap   <silent> <F4>         :execute ":ptag ".expand("<cword>")<CR>
nnoremap <Leader>fs /sub \<<C-r><C-w>\><CR>
if len($BASE_DIR) > 0
    set tags+=$BASE_DIR/tags
endif
if len($ENERGY_HOME) > 0
    set tags+=$ENERGY_HOME/tags
endif
if len($ENERGY_HOME_INDIA)
    set tags+=$ENERGY_HOME_INDIA/tags
endif
